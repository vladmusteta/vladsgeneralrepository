...
display_status() {
    ...
    local current_replicas_4=$(get_current_replicas "$DEPLOYMENT_4")
    if [ -z "$current_replicas_4" ]; then
        echo "Deployment '$DEPLOYMENT_4' not found or no replicas set."
    else
        echo "Current replicas for '$DEPLOYMENT_4': $current_replicas_4"
    fi
...
scale_deployments() {
    ...
    echo "Scaling '$DEPLOYMENT_4' to $target_replicas replicas..."
    kubectl scale deployment "$DEPLOYMENT_4" --replicas="$target_replicas" -n "$NAMESPACE"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to scale '$DEPLOYMENT_4'."
        success=1
    fi
