...
DEPLOYMENT_SERVER="home-assistant"
DEPLOYMENT_REDIS="mariadb"
...
display_status() {
    ...
    local redis_replicas=$(get_current_replicas "$DEPLOYMENT_REDIS")
    ...
    if [ -z "$redis_replicas" ]; then
        echo "Deployment '$DEPLOYMENT_REDIS' not found or no replicas set."
    else
        echo "Current replicas for '$DEPLOYMENT_REDIS': $redis_replicas"
    fi
    echo "------------------------------------------------------"
    echo ""
}
...
scale_deployments() {
    ...
    echo "Attempting to scale '$DEPLOYMENT_SERVER' and '$DEPLOYMENT_REDIS' to $target_replicas replicas..."
    ...
    kubectl scale deployment "$DEPLOYMENT_REDIS" --replicas="$target_replicas" -n "$NAMESPACE"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to scale '$DEPLOYMENT_REDIS'."
        return 1
    fi

    echo "Successfully scaled '$DEPLOYMENT_SERVER' and '$DEPLOYMENT_REDIS' to $target_replicas replicas."
    return 0
}
